 ...
 * Módulo de Instalação do Aplicativo (PWA) e Atualização de Cache
 */
    ...
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.register('sw.js')
        .then((reg) => {
          ...
          // Detecta novas versões no servidor
      ...
      // Recarrega se o controlador do service worker mudou
  ...
  bindInstallEvents() {
    ...
    window.addEventListener('appinstalled', () => {
      ...
      if (window.AppController && window.AppController.showToast) {
        window.AppController.showToast('✅ Viva Mulher instalado no celular!');
      }
    });
  }
};
