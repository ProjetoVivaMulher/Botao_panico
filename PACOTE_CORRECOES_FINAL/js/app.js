...
const STORAGE_KEY_LGPD = 'viva_mulher_lgpd_consent';
...
const AppController = {
  ...
  bindEvents() {
    if (this.dom.btnPanic) {
      this.dom.btnPanic.addEventListener('click', () => this.handlePanicButtonClick());
    }
    ...
    if (this.dom.btnAcceptConsent) {
      this.dom.btnAcceptConsent.addEventListener('click', () => this.handleAcceptConsent());
    }

    if (this.dom.btnCancelConsent) {
      this.dom.btnCancelConsent.addEventListener('click', () => this.handleCancelConsent());
    }

    if (this.dom.btnClearLocalData) {
      this.dom.btnClearLocalData.addEventListener('click', () => this.handleClearLocalData());
    }
  },
  ...
  handlePanicButtonClick() {
    if (!this.hasLgpdConsent()) {
      if (this.dom.lgpdConsentModal) {
        this.dom.lgpdConsentModal.classList.add('active');
      }
      return;
    }
  ...
  hasLgpdConsent() {
    return localStorage.getItem(STORAGE_KEY_LGPD) === 'true';
  },
  ...
  handleAcceptConsent() {
    try {
      ...
      console.warn('[AppController] Não foi possível gravar consentimento:', e);
    }
    if (this.dom.lgpdConsentModal) {
      this.dom.lgpdConsentModal.classList.remove('active');
    }
    this.showToast('✅ Consentimento LGPD registrado.');
    this.startPanicCountdown();
  },
  ...
  handleCancelConsent() {
    ...
    this.showToast('Acionamento suspenso (sem consentimento).');
  },

  handleClearLocalData() {
    if (confirm('Tem certeza que deseja apagar todos os contatos cadastrados, áudios e histórico deste celular?')) {
      try {
        localStorage.removeItem(STORAGE_KEY_LOGS);
        localStorage.removeItem('viva_mulher_emergency_contacts');
        localStorage.removeItem(STORAGE_KEY_LGPD);
      } catch (e) {
        console.error('[AppController] Erro ao limpar dados:', e);
      }
      this.renderContacts();
      this.renderLogs();
      this.showToast('🗑️ Todos os dados e registros locais foram apagados.');
    }
  },
